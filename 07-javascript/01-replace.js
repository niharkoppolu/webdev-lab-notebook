const replaceItems = (input) => {
  ...
  input = input.replace(/gold/gi, "SHINY"); //using a regular expression to match all instannces of gold with /g and case-insensitive /i
  input = input.replace(/wander/gi, "roam"); //primitives are immutable, need to re-assign value to string
